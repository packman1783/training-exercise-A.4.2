            ...
            try {
                ...
                if (LockB.lock2.tryLock(50, TimeUnit.MILLISECONDS)) {
                    ...
                    if (LockB.lock1.tryLock(50, TimeUnit.MILLISECONDS)) {
                        try {
                            System.out.println(Thread.currentThread().getName() + ": object lock2 lock1 monitor captured");
                            break;
                        } finally {
                            System.out.println(Thread.currentThread().getName() + ": object lock1 released");
                            LockB.lock1.unlock();
                        }
                    } else {
                        System.out.println(Thread.currentThread().getName() + ": object lock1 monitor not captured lock2 released");
                        LockB.lock2.unlock();
                    }
                }
                Thread.sleep(100);
